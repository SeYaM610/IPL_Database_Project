...
public class ClubWiseSalaryController {
    ...
    @FXML
    private TableView<ClubWiseSalaryController.ClubSalaryData> PlayerTable; // Table to display player data

    @FXML
    private TableColumn<ClubWiseSalaryController.ClubSalaryData, String> Club;

    @FXML
    private TableColumn<ClubWiseSalaryController.ClubSalaryData, Integer> Salary;
    ...
    public void PlayerInfoTable(Map<String, Integer> ClubWiseSalary) {
        // Create an ObservableList to hold table data
        ObservableList<ClubWiseSalaryController.ClubSalaryData> tableData = FXCollections.observableArrayList();

        // Populate ObservableList from the map
        for (Map.Entry<String, Integer> entry : ClubWiseSalary.entrySet()) {
            tableData.add(new ClubWiseSalaryController.ClubSalaryData(entry.getKey(), entry.getValue()));
        }
    ...
    public static class ClubSalaryData {
        private final String club;
        private final Integer salary;

        public ClubSalaryData(String club, Integer salary) {
            this.club = club;
            this.salary = salary;
        }
    ...
    private void alignColumnsToCenter() {
        alignColumnToCenter(Club);
        alignColumnToCenter(Salary);
    }

    // Helper method to center-align column content
    private <T> void alignColumnToCenter(TableColumn<ClubSalaryData, T> column) {
        column.setCellFactory(tc -> {
            TableCell<ClubSalaryData, T> cell = new TableCell<>() {
                @Override
